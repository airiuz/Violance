        ...
        for transport in transports:
            ...
            if len(plate_coord)>0:
                ...
                if x2<width and y2<height:
                    ...
                    xk=abs(x1-x2)
                    yk=abs(y1-y2)
                    cv2.rectangle(frame, (x1+int(bbox[0]),y1+int(bbox[1])), (x1+int(bbox[0])+xk,y1+int(bbox[1])+yk),(0,0,255),1)
                    if result is None:
                        result = yk
            ...
            if bbox[1]<y_zebra and y_zebra<bbox[3] and count>3:
                ...
                if len(plate_coord)>0:
                    ...
                    if len(plate_number) == 8:
                        cursor.execute("SELECT COUNT(*) from avtomobil WHERE number=?", (plate_number,))
                        if cursor.fetchall()[0][0] == 0:
                            path = f"violance base/rasm{self.son}.jpg"
                            cursor.execute("INSERT INTO avtomobil(path, number, vaqt) VALUES(?, ?, ?)", (path, plate_number, time))
                            baza.commit()
